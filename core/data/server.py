...
class Server(DataObject):
    ...
    def __post_init__(self):
        ...
        with DBConnection() as cursor:
            ...
            cursor.execute('SELECT embed_name, embed FROM message_persistence WHERE server_name = ?', (self.name, ))
    ...
    def ban(self, player: Player, reason: str = 'n/a', period: int = 30*86400):
        self.sendtoDCS({
            "command": "ban",
            "id": player.id,
            "period": period,
    ...
    async def setEmbed(self, embed_name: str, embed: discord.Embed, file: Optional[discord.File] = None,
        ...
        async with self._lock:
            ...
            if not message:
                ...
                self.embeds[embed_name] = message
                with DBConnection() as cursor:
                    cursor.execute('INSERT INTO message_persistence (server_name, embed_name, embed) VALUES (?, ?, ?) '
